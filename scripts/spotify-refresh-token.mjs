...
//
// 整個流程都在本機完成：這支 script 會起一個 127.0.0.1:8888 的小伺服器接授權結果，
// 換到的 refresh token 直接印在終端機。用法:
//   1. 到 https://developer.spotify.com/dashboard 建立 App，記下 Client ID / Client Secret
//   2. 在 App 的 Edit Settings → Redirect URIs 加入 http://127.0.0.1:8888/callback 並儲存
//      （Spotify 現在會擋字面上的 localhost，要求用回環 IP 127.0.0.1，見
...
// 只有本機回環位址才起伺服器收 code；指到別的位址就只印授權網址，code 自己接
if (new URL(REDIRECT_URI).hostname !== "127.0.0.1") {
