...
**雜談 (Thoughts)**
Discord 機器人支援 `/碎碎念` 斜線指令；提交內容會在 `app/api/discord/route.ts` 中驗證（Ed25519，透過 `tweetnacl`）並儲存於 Vercel KV。`/thoughts` 頁面會將這些內容與從 Threads API（`app/lib/threads.ts`）取得的貼文合併，依時間戳由新到舊排序，並在每次新增 Discord 貼文時重新驗證。若無遠端資料可用，則回退至 `app/data.ts` 中的靜態 `THOUGHTS` 陣列。
...
app/
  api/
    discord/route.ts                 Discord 互動 Webhook（斜線指令 → KV）
  components/
...
public/
  ...
  icon/               自訂 SVG 圖示
scripts/
  register-command.mjs    註冊 Discord「/碎碎念」斜線指令的一次性腳本
...
`/thoughts` 頁面與 Discord 整合所需（詳見 `.env.local`）：

| 變數 | 用途 |
|---|---|
| `DISCORD_APP_ID`、`DISCORD_BOT_TOKEN` | 註冊斜線指令（`scripts/register-command.mjs`） |
| `DISCORD_PUBLIC_KEY` | 驗證 `app/api/discord/route.ts` 中的互動簽章 |
...
部署於 Vercel；推送至 `main` 分支會觸發新的正式環境部署。自訂網域設定於 Vercel 專案中（`CNAME` 檔案是先前 GitHub Pages 設定所留下的遺留產物）。Discord Webhook 事件透過 `/api/discord` 路由接收，這要求在 Discord 開發者後台中將此應用程式的 Vercel 部署網址註冊為互動端點。
